...
// The bundled pre-trained model ("three-skill" model: Jabber poems + single-variable
// algebra + sorting) ships with the site so the app works out of the box — inference
// and inspection run immediately without anyone having to train first. The one model
// shows three things at once: text generation (poems), confident-but-wrong arithmetic
// (the hallucination lesson), and genuinely-learned sorting (generalises to unseen
// inputs — "real" reasoning, with a grokking story). This module is the single source
// of truth for fetching and installing it — shared by the main app (auto-load at
// start + manual buttons) and the explainer's fallback.
...
export async function fetchBundledModel(): Promise<SavedModel | null> {
  const res = await fetch(import.meta.env.BASE_URL + 'multitask-model.json')
